...
try {
  ...
  const plusBtnCandidates = [
    'header a[href*="new"]',
    'a[href="/recipes/new"]',
    'header button[aria-label*="add" i]',
    'header button[aria-label*="create" i]',
    'header button:has-text("+")',
    'button:has-text("+")',
    'a:has-text("+")',
    '[aria-label*="add recipe" i]',
    '[aria-label*="create recipe" i]'
  ];
  let plusFound = null;
  ...
  try {
    ...
    let titleSel = null;
    for (const sel of titleInputCandidates) {
      if (await page.locator(sel).count() > 0) { titleSel = sel; break; }
    }
    ...
    currentStep = 'step-07';
    await page.fill(titleSel, 'Test Pasta');
    const descCandidates = ['textarea[name="description"]', 'textarea[placeholder*="description" i]', 'textarea[id*="description" i]'];
    ...
    const ingInputs = page.locator('input[name*="ingredient" i], input[placeholder*="ingredient" i]');
    ...
    const stepsArea = page.locator('textarea[name="steps"], textarea[name="instructions"], textarea[placeholder*="step" i], textarea[placeholder*="instruction" i]').first();
    if (await stepsArea.count() > 0) {
      await stepsArea.fill('Boil pasta\nFry garlic\nCombine');
    } else {
      const stepInputs = page.locator('input[name*="step" i]');
    ...
    if (await submitBtn.count() === 0) throw new Error('No submit button');
    ...
    log('step-08', sawTestPasta ? 'PASS' : 'FAIL',
  ...
  await page.waitForTimeout(500);
    ...
    if (await editBtn.count() === 0) {
      log('step-10', 'FAIL', 'No edit button on detail page');
    } else {
      await editBtn.click();
      await page.waitForLoadState('networkidle').catch(()=>{});
      await page.waitForTimeout(800);
      await shot('10a-edit-form.png');
      const titleSel2 = (await page.locator('input[name="title"]').count() > 0) ? 'input[name="title"]' : null;
      if (!titleSel2) {
        log('step-10', 'FAIL', 'No title input on edit page');
      } else {
        await page.fill(titleSel2, 'Test Pasta v2');
